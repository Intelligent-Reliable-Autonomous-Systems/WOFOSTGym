...
class Pernnial_Multi_Limited_NPKW_Env(Multi_NPK_Env, LNPKW):
    ...
    def _take_action(self, action: int):
        ...
        if action >= 3 * self.num_fert+1:
            ...
            [self.models[i]._send_signal(signal=pcse.signals.irrigate, amount=i_amount, \
                                    efficiency=self.irrig_effec) for i in range(self.num_farms)]
            return (n_amount, p_amount, k_amount, i_amount)
        ...
        if (action-1) // self.num_fert == 0:
            ...
            [self.models[i]._send_signal(signal=pcse.signals.apply_npk, \
                                    N_amount=n_amount, N_recovery=self.n_recovery) for i in range(self.num_farms)]
        elif (action-1) // self.num_fert == 1:
            p_amount = self.fert_amount * (( (action-1) % self.num_fert)+1) 
            [self.models[i]._send_signal(signal=pcse.signals.apply_npk, \
                                    P_amount=p_amount, P_recovery=self.p_recovery) for i in range(self.num_farms)]
        elif (action-1) // self.num_fert == 2:
            k_amount = self.fert_amount * (( (action-1) % self.num_fert)+1) 
            [self.models[i]._send_signal(signal=pcse.signals.apply_npk, \
                                        K_amount=k_amount, K_recovery=self.k_recovery) for i in range(self.num_farms)]
        return (n_amount, p_amount, k_amount, i_amount)
