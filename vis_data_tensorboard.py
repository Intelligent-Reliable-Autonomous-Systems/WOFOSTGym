"""
Module for visualizing data from tensorboard

Written by Will Solow, 2024

To run: python3 vis_data_tensorboard.py
"""
...
def plot_scalars(data, names, args, log_scale=False):
    ...
    for i,d in enumerate(data.items()):
        ...
        plt.fill_between(steps, avg+std, avg-std, color=utils.COLORS[i],alpha=.6)
...
def plot_scalars_two_log(data, names, args):
    """
    Plot scalar data on a single matplotlib plot.
    ...
    for i,d in enumerate(data.items()):
        ...
        if i > 2:
            ...
            formatter.set_powerlimits((4, 4))  # Only show scientific notation for large scales
            ax[1].yaxis.set_major_formatter(formatter)
            formatter = ticker.ScalarFormatter(useMathText=True)
            formatter.set_powerlimits((4, 6))  # Only show scientific notation for large scales
    ...
    ax[0].set_yticks([-1e6, -1e4, -1e2, 0, 1e2, 1e4], labels=[f"$-10^6$", f"$-10^4$", f"$-10^2$", f"$0$", f"$10^2$",f"$10^4$" ], fontsize=lb_size)
...
def plot_scalars_two_default(data, names, args):
    """
    Plot scalar data on a single matplotlib plot.
    ...
    rews = [1586094, 725497 , 53314, 141584 ]
    ...
    for i,d in enumerate(data.items()):
        ...
        if i > 2:
            ...
            formatter.set_powerlimits((4, 6))  # Only show scientific notation for large scales
            ax[1].xaxis.set_major_formatter(formatter)
            ax[1].plot(steps, avg, label=names[i],color=utils.COLORS[i-3])
            ax[1].fill_between(steps, avg+std, avg-std, color=utils.COLORS[i-3],alpha=.4)
...
def wandb_plot_scalars():
    ...
    ax.legend(handlelength=1,ncol=2,)
    plt.savefig(f"{args.save_name}.png", bbox_inches='tight')
    plt.show()
    
def plot_scalars_log_bar(data, names, args):
    """
    Plot scalar data on a single matplotlib plot.
    ...
    for i in x:
        ax[1].bar(x[i] , loc_mean[i], color=utils.COLORS[i],alpha=.6)
...
def load_bar_data(directories=["data/runs"]):
    """
    ...
    # Group all files
    grouped_files = [[],[],[],[]]
    prefixes = ['data/runs/potato_all', 'data/runs/potato_rain_totn', 'data/runs/potato_totn', 'data/runs/potato_rain']
    for n in npz_files:
        if prefixes[0] in n:
...
def plot_scalars_log_hist(data, names, args):
    """
    Plot scalar data on a single matplotlib plot.
    ...
    #colormaps = [fix_colormap(plt.cm.Greens), fix_colormap(plt.cm.Purples), fix_colormap(plt.cm.Oranges), fix_colormap(plt.cm.Blues)]
    ...
    for i in range(nrows):
        ...
        ax[1].pcolormesh(X, Y, row_data.reshape(1, -1), cmap=colormaps[i], shading='flat', vmin=Z[i].min(), vmax=Z[i].max(), edgecolors=(0, 0, 0, 0.2), linewidth=0.5)
    ax[1].set_xlabel("Weeks", fontsize=fnt_size)
    ax[1].tick_params(axis='x', labelsize=lb_size)
    ax[1].set_yticks([.5, 1.5, 2.5, 3.5], labels=["N", "P", "K", "W"][::-1], fontsize=fnt_size)
    ...
    x = np.load("data/runs/pear_threshold_wk_open.npz", allow_pickle=True)
    rewards = x["rewards"]
    splits = np.where(x['dones'])[0]+1
    split_rew = np.split(rewards, [s for s in splits if 0 < s < len(rewards)])
    rews = np.mean([np.sum(r) for r in split_rew])
    ax[0].axhline(rews+14000, color='green', linestyle='--', label='Threshold NW')
...
def load_hist_data(directories=["data/runs"]):
    # Get all NPZ files 
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
    ...
    # Group all files
    grouped_files = []
    prefixes = ['data/runs/pear_threshold']
    for n in npz_files:
        if prefixes[0] in n:
...
def plot_scalars_default_yield(data, names, args):
    ...
    formatter = ticker.ScalarFormatter(useMathText=True)
    formatter.set_powerlimits((2, 2))  # Only show scientific notation for large scales
    ax[1].yaxis.set_major_formatter(formatter)
    ax[1].tick_params(axis='y', labelsize=lb_size)
    ax[1].tick_params(axis='x', labelsize=lb_size)
    ...
    for i in range(means.shape[0]):
        for j in range(means.shape[1]):
            ax[1].bar(x[j] + i * (width), means[i, j], width=width, 
               color=utils.COLORS[i], alpha=0.5, capsize=5)
            # Only add the positive error (top half) for each bar and set the capsize only on the top
            ax[1].errorbar(x[j] + i * (width), means[i,j], yerr=stds[i,j], fmt='none', color='k',capsize=3, alpha=.5, lolims=True, label='Top cap only')
...
def plot_default_yield(data, names, args):
    ...
    for i in range(obs_means.shape[0]):
        for j, x in enumerate(xs):
            ...
            ax[0].fill_between(np.arange(obs_means.shape[1]), obs_means[i,:,x]+obs_stds[i,:,x], obs_means[i,:,x]-obs_stds[i,:,x], color=utils.COLORS[i], alpha=.1)

    handles = [Line2D([0], [0], color='k', marker=utils.MARKERS[i] ,alpha=0.4, lw=2, label=f'Farm {i+1}') for i in range(5)]
    ...
    # Set up the positions for the bars
    #x = np.arange(means.shape[1])-.3 # Positions of the bars within each group
    x = np.arange(wso_mean.shape[1]) - .3
    width = .3 # Width of the bars
    ...
    for i in range(wso_mean.shape[0]):
        for j in range(wso_mean.shape[1]):
            ...
            if wso_mean[i, j] > means[i, j]:
                ax[1].bar(x[j] + i * (width), wso_mean[i, j], width=width, 
               color=utils.COLORS2[i], alpha=alpha, capsize=5, zorder=1)
                ax[1].bar(x[j] + i * (width), means[i, j], width=width, 
               color=utils.COLORS[i], alpha=alpha, capsize=5, zorder=2, hatch="xx")
            else:
                ax[1].bar(x[j] + i * (width), wso_mean[i, j], width=width, 
                color=utils.COLORS2[i], alpha=alpha, capsize=5, zorder=2)
                ax[1].bar(x[j] + i * (width), means[i, j], width=width, 
                color=utils.COLORS[i], alpha=alpha, capsize=5, zorder=1, hatch="xx")
            #ax[1].errorbar(x[j] + i * (width), means[i,j], yerr=stds[i,j], fmt='none', color='k',capsize=3, alpha=.5, lolims=True, label='Top cap only')
    '''    # Plot the bars with error bars
    for i in range(means.shape[0]):
        for j in range(means.shape[1]):
            ax[1].bar(x[j] + i * (width), means[i, j], width=width, 
               color=utils.COLORS[i], alpha=0.5, capsize=5, zorder=1)
            # Only add the positive error (top half) for each bar and set the capsize only on the top
            ax[1].errorbar(x[j] + i * (width), means[i,j], yerr=stds[i,j], fmt='none', color='k',capsize=3, alpha=.5, lolims=True, label='Top cap only')'''
...
def load_multi_wso_data(directories=["data/runs"], prefixes=['data/runs/sunflower_cost']):
    # Get all NPZ files 
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
...
def load_single_wso_data(directories=["data/runs"], prefixes=['data/runs/sunflower_single']):
    # Get all NPZ files 
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
...
def plot_bc_yield(directories=["data/runs"], prefixes=['data/runs/wheat_threshold_wk']):
    # Get all NPZ files 
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
...
if __name__ == "__main__":
    ...
    #Jujube and wheat
    log_dirs = ["data/Jujube_Rand/DQN/perennial-lnpkw-v0__rl_utils__1__1738094765",
                "data/Jujube_Rand/PPO/perennial-lnpkw-v0__rl_utils__1__1738094738",
                "data/Jujube_Rand/SAC/perennial-lnpkw-v0__rl_utils__1__1738094745",
                "data/Wheat_Rand/DQN/lnpkw-v0__rl_utils__1__1738094724",
                "data/Wheat_Rand/PPO/lnpkw-v0__rl_utils__1__1738094724",
                "data/Wheat_Rand/SAC/lnpkw-v0__rl_utils__1__1738094724"]
    names = ["DQN", "PPO", "SAC"]
    scalar_name = "charts/average_reward"
    data = load_scalars_from_runs(log_dirs, scalar_name)
    args.save_name = "data/figs/jujube_wheat_rand"
    plot_scalars_two_default(data, names, args)

    # Maize and pear threshold 
    log_dirs = ["data/Pear_Threshold_WK_Rand/DQN/perennial-lnpkw-v0__rl_utils__1__1738185068",
                "data/Pear_Threshold_WK_Rand/PPO/perennial-lnpkw-v0__rl_utils__1__1738185049",
                "data/Pear_Threshold_WK_Rand/SAC/perennial-lnpkw-v0__rl_utils__1__1738191552",
                "data/Maize_Threshold_WK_Rand/DQN/lnpkw-v0__rl_utils__1__1738184848",
                "data/Maize_Threshold_WK_Rand/PPO/lnpkw-v0__rl_utils__1__1738184826",
                "data/Maize_Threshold_WK_Rand/SAC/lnpkw-v0__rl_utils__1__1738193770",
                ]
    names = ["DQN", "PPO", "SAC"]
    scalar_name = "charts/average_reward"
    data = load_scalars_from_runs(log_dirs, scalar_name)
    args.save_name = "data/figs/pear_maize_threshold"
    plot_scalars_two_log(data, names, args)
    
    # PPO runoff
    log_dirs = ["data/Potato_Limited_WK_Rand/PPO/lnpkw-v0__rl_utils__1__1738213380", # All
                "data/Potato_Limited_WK_Rand/PPO/lnpkw-v0__rl_utils__1__1738213406", # No Rain
                "data/Potato_Limited_WK_Rand/PPO/lnpkw-v0__rl_utils__1__1738213411", # No total N/NAvail
                "data/Potato_Limited_WK_Rand/PPO/lnpkw-v0__rl_utils__1__1738213417", #No total N/Rain 
                ]
    names = ["Neither", "RAIN", "TOTN", "RAIN+TOTN"]
    scalar_name = "charts/average_reward"
    data = load_scalars_from_runs(log_dirs, scalar_name)
    args.save_name = "data/figs/potato_limited_threshold"
    plot_scalars_log_bar(data, names, args)
    ...
    log_dirs = ["data/Pear_Threshold_WK_Rand/DQN/perennial-lnpkw-v0__rl_utils__1__1739915702",
                "data/Pear_Threshold_WK_Rand/PPO/perennial-lnpkw-v0__rl_utils__1__1739915694",
                "data/Pear_Threshold_WK_Rand/SAC/perennial-lnpkw-v0__rl_utils__1__1739915713"]
    names = ["DQN", "PPO", "SAC"]
    scalar_name = "charts/average_reward"
    data = load_scalars_from_runs(log_dirs, scalar_name)
    args.save_name = "data/figs/pear_threshold"
    plot_scalars_log_hist(data, names, args)

    '''#Sunflower Cost and sunflower WK
    log_dirs = ["data/Sunflower_Multi_Cost/DQN/multi-lnpkw-v0__rl_utils__1__1739134603",
                "data/Sunflower_Multi_Cost/PPO/multi-lnpkw-v0__rl_utils__1__1738984306",
                "data/Sunflower_Multi_Cost/SAC/multi-lnpkw-v0__rl_utils__1__1738984329",
                "data/Sunflower_Multi_WK/DQN/multi-lnpkw-v0__rl_utils__1__1739134640",
                "data/Sunflower_Multi_WK/PPO/multi-lnpkw-v0__rl_utils__1__1739134642",
                "data/Sunflower_Multi_WK/SAC/multi-lnpkw-v0__rl_utils__1__1739130329"
