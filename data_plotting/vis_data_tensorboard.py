...
def plot_reward_log_histprobs(data, names, args):
    """
...
def plot_scalars_log_hist(data, names, args):
    ...
    x = np.load("experiments/runs/pear_threshold_wk_open.npz", allow_pickle=True)
    rewards = x["rewards"]
    ...
    ax[0].axhline(rews, color='green', linestyle='--', label='Threshold NW')
...
def load_hist_data(directories=["experiments/runs"]):
    # Get all NPZ files 
    npz_files = []
    for directory in directories:
        for root, _, files in os.walk(directory):
    ...
    # Group all files
    grouped_files = []
    prefixes = ['experiments/runs/pear_threshold']
    for n in npz_files:
        if prefixes[0] in n:
...
if __name__ == "__main__":
    ...
    plot_reward_log_histprobs(data, names, args)
